 ...
 * 1.00				23 Dec 2020		Doug Humberd		Allows a user to select Invoices for PDF generation
 * Version			Date			Author				Remarks
 * 1.5.1				07 Apr 2022		John Kumuyi  		FSP-64 present credits & payments clearly on Customer Invoice
 * Version			Date			Author				Remarks
 * 1.5.2			07 Apr 2022		John Kumuyi  		FSP-53 ability to select template to send Customer Invoice
 *
 */
 ...
 */
 ...
 function scg_invpdf_listInvoices(request, response){
     if (request.getMethod()=='GET') {
         try	{
             var form = nlapiCreateForm('One Off Invoice Bulk Processing');
            ...
            /*
             var primary = form.addFieldGroup( 'primary', 'Primary');
 
             //Form Fields
             fld = form.addField('custpage_tran_start_date', 'date', 'Earliest Tran Date', null, 'primary');
             fld.setDefaultValue(tranStartDate);
 
             fld = form.addField('custpage_tran_end_date', 'date', 'Latest Tran Date', null, 'primary');
             fld.setDefaultValue(tranEndDate);
 
             fld = form.addField('custpage_customer', 'select', 'Customer', 'customer', 'primary');
             fld.setDefaultValue(customer);
             
             fld = form.addField('custpage_subsidiary', 'select', 'Subsidiary', 'subsidiary', 'primary');
             fld.setDefaultValue(subsidiary);
             fld.setBreakType('startcol');
 
             fld = form.addField('custpage_status', 'select', 'Status', 'status', 'primary');
             ...
             var fld = form.addField('custpage_success', 'checkbox', 'Include Previously Successful Transactions', null, 'primary');
             fld.setDefaultValue(success);
 
             var fld = form.addField('custpage_failed', 'checkbox', 'Include Previously Failed Transactions', null, 'primary');
             fld.setDefaultValue(failed);
 
             var fld = form.addField('custpage_email', 'checkbox', 'Email Invoice', null, 'primary');
             fld.setDefaultValue((email == 'F') ? 'F' : 'T');
             fld.setBreakType('startcol');
 
             var fld = form.addField('custpage_mail', 'checkbox', 'Mail Invoice', null, 'primary');
             fld.setDefaultValue((mail == 'F') ? 'F' : 'T');
             
             var fld = form.addField('custpage_portal', 'checkbox', 'Portal Submit', null, 'primary');
             fld.setDefaultValue((portal == 'F') ? 'F' : 'T');
 
             //var fld = form.addField('custpage_consortium', 'checkbox', 'Bill To Consortium');
             //fld.setDefaultValue((consortium == 'F') ? 'F' : 'T');
            */
             var fld = form.addField('custpage_count', 'integer', 'Invoice Count', null, 'primary');
             fld.setDisplayType('inline');
             fld.setDefaultValue(0);
             fld.setBreakType('startcol');
 
             var fld = form.addField('custpage_total', 'currency', 'Invoice Total', null, 'primary');
             fld.setDisplayType('inline');
             fld.setDefaultValue(0);
 
             fld = form.addField('custpage_filter_changed', 'checkbox', 'Filter Changed', null, 'primary');
             fld.setDefaultValue('F');
             fld.setDisplayType('hidden');
 
             var urlParamId = 'custpage_url_params';
             fld = form.addField(urlParamId, 'text', 'URL Parameters', null, 'primary');
             fld.setDisplayType('hidden');
            
             var Message = form.addFieldGroup( 'message', 'Message');

             // Email Template

             fld = form.addField('custpage_emailtemplate', 'select', 'Email Template', 'emailtemplate', 'message');
             fld.setDefaultValue(14);
             ...
             invList.addField('custpage_inv_subsidiary', 'select', 'Subsidiary', 'subsidiary').setDisplayType('inline');
             ...
             var resultCount = 0;
             //do {
                 var results = scg_invpdf_getInvoices(customer, subsidiary, status, tranStartDate, tranEndDate, failed, success, email, mail, portal, minInternalId);
                 //var results = scg_invpdf_getInvoices(customer, subsidiary, status, tranStartDate, tranEndDate, failed, success, email, portal, minInternalId);
                 ...
                 for(var x=0; results && x < results.length; x++) {
                     ...
                     invData[invCount]['custpage_inv_subsidiary'] = results[x].getValue('subsidiarynohierarchy');
         ...
         try {
             ...
             var invIds = [];
             var emailId = request.getParameter('custpage_emailtemplate');
             ...
             if (invIds) {
                 var scriptSched = nlapiScheduleScript('customscript_scg_process_invoices_ss', 'customdeploy_scg_process_invoices_ss', {'custscript_process_inv_ids': JSON.stringify(invIds), 'custscript_process_inv_email_id': emailId});
  ...
  */
 function scg_invpdf_getInvoices(customer, subsidiary, status, tranStartDate, tranEndDate, failed, success, email, mail, portal, minInternalId) {
 //function scg_invpdf_getInvoices(customer, subsidiary, status, tranStartDate, tranEndDate, failed, success, email, portal, minInternalId) {
    ...
    //      }
    //      if (portal == 'T') {
    //          deliveryMethodFormula += ' OR INSTR({custbody_invoice_delivery_type}, \'' + DELIVERY_TYPE_TEXT_PORTAL + '\') > 0';
    //      }
    //  } else if (mail == 'T') {
    ...
    //      }
    //  } else if (portal == 'T') {
    //      deliveryMethodFormula += 'INSTR({custbody_invoice_delivery_type}, \'' + DELIVERY_TYPE_TEXT_PORTAL + '\') > 0'; 
    //  } else {
    ...
    //      //deliveryMethodFormula += 'INSTR({custbody_invoice_delivery_type}, \'' + DELIVERY_TYPE_TEXT_EMAIL + '\') = 0 AND INSTR({custbody_invoice_delivery_type}, \'' + DELIVERY_TYPE_TEXT_PORTAL + '\') = 0';
    //  }
    ...
    //  filters.push(new nlobjSearchFilter('subsidiary', null, 'anyof', (subsidiary) ? subsidiary : '@ALL@'));
    ...
    var search = nlapiLoadSearch('transaction', 'customsearch1226');
    ...
    do {
        ...
        if( !resultSet || resultSet.length == 0 ) break;
