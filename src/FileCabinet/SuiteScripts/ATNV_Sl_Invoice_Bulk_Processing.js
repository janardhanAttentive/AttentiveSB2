 ...
 */
		...
		function(record, runtime, error, search, task, serverWidget, dialog, redirect, format, url) {
			function onRequest(context) {
				...
				log.debug('intEmailTemplate',intEmailTemplate);
					...
					{
					var inTranStartDate = context.request.parameters.custpage_tran_start_date;
					var inTransEndDate = context.request.parameters.custpage_tran_end_date;
					var inCustomer = context.request.parameters.custpage_customer;
					var inSubsidiary = context.request.parameters.custpage_subsidiary;
					var inStatus = context.request.parameters.custpage_status;
					var inSuccess = context.request.parameters.custpage_success;
					var inFailed = context.request.parameters.custpage_failed;
					var inEmail = context.request.parameters.custpage_email;
					var inMail = context.request.parameters.custpage_mail;
					var inPortal = context.request.parameters.custpage_portal;
					var inPageCount = context.request.parameters.custpage_count;
					var inPageTotal = context.request.parameters.custpage_total;

												
						var form = serverWidget.createForm({title: 'Invoice Bulk Processing',hideNavBar: false});
	...
	var fldEarliestTranDate = form.addField({id: 'custpage_tran_start_date',type: serverWidget.FieldType.DATE,label: 'Earliest Tran Date',container:'primary'});
    var fldLatestTranDate = form.addField({id: 'custpage_tran_end_date',type: serverWidget.FieldType.DATE,label: 'Latest Tran Date',container:'primary'});
	var fldCustomer = form.addField({id: 'custpage_customer',type: serverWidget.FieldType.SELECT,source:'customer',label: 'Customer',container:'primary'});
	var fldSubsidiary = form.addField({id: 'custpage_subsidiary',type: serverWidget.FieldType.SELECT,source:'subsidiary',label: 'Subsidiary',container:'primary'});
	fldSubsidiary.updateBreakType({breakType : serverWidget.FieldBreakType.STARTCOL});
	var fldStatus = form.addField({id: 'custpage_status',type: serverWidget.FieldType.SELECT,source:'status',label: 'status',container:'primary'});
	
				fldStatus.addSelectOption({value : 'CustInvc:A',text : 'Open'});
				fldStatus.addSelectOption({value : 'CustInvc:B',text : 'Paid In Full'});
				fldStatus.addSelectOption({value : '@ALL@',text : 'All'});
	
	var fldSucessTrans = form.addField({id: 'custpage_success',type: serverWidget.FieldType.CHECKBOX,label: 'Include Previously Successful Transactions',container:'primary'});
	var fldFailedTrans = form.addField({id: 'custpage_failed',type: serverWidget.FieldType.CHECKBOX,label: 'Include Previously Failed Transactions',container:'primary'});
	var fldEmailInvoice = form.addField({id: 'custpage_email',type: serverWidget.FieldType.CHECKBOX,label: 'Email Invoice',container:'primary'});
		fldEmailInvoice.updateBreakType({breakType : serverWidget.FieldBreakType.STARTCOL});

	var fldMailInvoice = form.addField({id: 'custpage_mail',type: serverWidget.FieldType.CHECKBOX,label: 'Mail Invoice',container:'primary'});
	var fldPortalSubmit = form.addField({id: 'custpage_portal',type: serverWidget.FieldType.CHECKBOX,label: 'Portal Submit',container:'primary'});
	var fldInvoiceCount = form.addField({id: 'custpage_count',type: serverWidget.FieldType.INTEGER,label: 'Invoice Count',container:'primary'}).updateDisplayType({displayType: serverWidget.FieldDisplayType.INLINE});;
			fldInvoiceCount.updateBreakType({breakType : serverWidget.FieldBreakType.STARTCOL});
		 //fldInvoiceCount.getField({id: 'custpage_count'}).updateDisplayType({displayType: serverWidget.FieldDisplayType.INLINE});

	var fldInvoiceTotal = form.addField({id: 'custpage_total',type: serverWidget.FieldType.CURRENCY,label: 'Invoice Total',container:'primary'}).updateDisplayType({displayType: serverWidget.FieldDisplayType.INLINE});
		// fldInvoiceTotal.getField({id: 'custpage_total'}).updateDisplayType({displayType: serverWidget.FieldDisplayType.INLINE});


		
			form.addFieldGroup({id : 'message',label : 'Message'});
			...
			// create header fields
			 ...
			 var fldEmailTemplate = form.addField({id: 'custpage_emailtemplate',type: serverWidget.FieldType.SELECT,source:'emailtemplate',label: 'Email Template',container:'message'});
					...
					var Transdata = TransactionData[0];
					var invTotal = TransactionData[1];
					var invCount = TransactionData[2];
					
					if(invTotal)
					{
							fldInvoiceTotal.defaultValue = invTotal.toFixed(2);	

					}
					if(invCount)
					{
							fldInvoiceCount.defaultValue = invCount;	

					}

				log.debug({title: 'Transaction_data',details: JSON.stringify(Transdata)});
	...
	var data =[];
			...
			var invCount = 0;
			 ...
			 arrColumns.push(search.createColumn({name: "internalid",sort: search.Sort.ASC,label: "Internal ID"}));
						  ...
						  arrColumns.push(search.createColumn({name: "total", label: "total"}));
		...
		while ( j < currentRange.length ) {
			...
			var objLine = {
						...
						invAmount: result.getValue('total')
						 };
			   data.push(objLine);
			            invTotal += parseFloat(result.getValue('total'));
